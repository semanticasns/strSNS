...
// REST REFSET Extensión Medicamentos
	...
	while ($fila = $sql->fetch(PDO::FETCH_ASSOC)) { 
		...
		$sql = $dbConn->prepare("select * from snomedct_drug.".$valor5." ");		
		$sql->execute();	
		$result = $sql->rowCount();	
		$myfile = fopen($fichero, 'w') or die("Imposible crear fichero!");		
		$valor6 = $sql->setFetchMode(PDO::FETCH_ASSOC);	
		ini_set('memory_limit', '-1');
		fwrite ($myfile,json_encode( $valor6=$sql->fetchAll(),JSON_UNESCAPED_UNICODE ));	
		fclose($myfile);		
