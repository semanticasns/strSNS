...
// REST Mapeo SNOMED CT a ICD-O WHO
	...
	$sql = $dbConn->prepare("select * from der2_srefset_simplemapsnapshot_int a inner join sct2_description_snapshot_global b
		on a.referencedComponentId = b.conceptId 
		where a.refsetId = 446608001 and b.typeId = 900000000000003001 and b.languageCode != 'en' and b.active = 1 and 
...
//	$sql->bindValue(1, "$filtro", PDO::PARAM_INT);
	...
	echo "<h2>ICD-O WHO Map to SNOMED CT</h2>" ;
	echo "<table>" ;
	...
	while ($qr = $sql->fetch(PDO::FETCH_ASSOC)) : 	
		...
		echo "<tr><td>".$var09."</td><td>".$var06."</td><td>".$var07."</td><td>".$var02."</td><td>".$var01."</td><td>".$var03."</td><td>".$var04."</td><td width=5%>".$var08."</td></tr>" ;
	endwhile ;
	echo "</table>" ;	
